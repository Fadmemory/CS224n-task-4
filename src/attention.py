...
def apply_rotary_emb(x, rope_cache):
    ...
    rotated_x = None
    ### YOUR CODE HERE ###
    seq_len = x.shape[-2]  # x shape: (..., seq_len, head_dim)
    rope_cache = rope_cache[:seq_len]  # shape: (seq_len, head_dim/2, 2)
    # x shape: (..., seq_len, head_dim) -> (..., seq_len, head_dim/2, 2)
    x_reshaped = x.reshape(*x.shape[:-1], -1, 2)
    x_complex = torch.view_as_complex(x_reshaped)  # shape: (..., seq_len, head_dim/2)
    cos_vals = rope_cache[..., 0]  # shape: (seq_len, head_dim/2)
    sin_vals = rope_cache[..., 1]  # shape: (seq_len, head_dim/2)
    rotation_complex = torch.complex(cos_vals, sin_vals)  # shape: (seq_len, head_dim/2)
    rotated_complex = x_complex * rotation_complex
    rotated_x = torch.view_as_real(rotated_complex)  # shape: (..., seq_len, head_dim/2, 2)
